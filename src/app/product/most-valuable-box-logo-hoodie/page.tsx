...
export default function BoxLogoHoodieProductPage() {
  const [selectedSize, setSelectedSize] = useState("");
  const [isLoading, setIsLoading] = useState(false);

  // Find the product
  const product = products.find(p => p.slug === "most-valuable-box-logo-hoodie");
  
  if (!product) {
    return <div>Product not found</div>;
  }

  const currentImage = mediaUrl(product.media?.[0] || "");
